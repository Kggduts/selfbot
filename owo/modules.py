...
class Modules:
	...
	async def oauth_topgg(self, oauth, oauth_req):
		retry_times = 0
		while retry_times < int(self.client.data.config.error_retry_times):
	...
	async def vote_topgg(self):
		...
		topgg = await self.oauth_topgg(oauth, oauth_req)
		async with webdriver.Chrome(options=options) as driver:
			...
			await button.click()
			self.client.logger.info(f"Loaded OwO vote page on top.gg")
			await asyncio.sleep(20)
			button = await driver.find_element(by=By.XPATH, value=".//button[contains(text(),'Vote')]")
			await button.click()
			self.client.logger.info(f"Voted OwO on top.gg")
			await asyncio.sleep(20)

	async def claim_daily(self):
		if self.client.data.available.selfbot and int(self.client.data.timeout.daily) - time.time() <= 0:
	...
	async def claim_submit_huntbot(self):
		if self.client.data.available.selfbot and int(self.client.data.timeout.huntbot) - time.time() <= 0:
	...
	async def check_glitch(self):
		if self.client.data.available.selfbot and self.client.data.timeout.glitch - time.time() <= 0:
	...
	async def solve_image_captcha(self, image, captcha, lenghth, wrong_answer):
		...
		for api_key in self.client.data.config.image_captcha['twocaptcha']:
			...
			await self.client.notification.notify()
		if result:
			await self.client.data.owo.send(result['code'])
			self.client.logger.info(f"Sent {result['code']}")
			self.client.data.stat.command += 1
			try:
				captcha_verification = await self.client.wait_for("message", check = lambda message: message.channel.id == self.client.data.owo.dm_channel.id and any(text in message.content for text in ['👍', '🚫']), timeout = 5)
		...
		while retry_times < int(self.client.data.config.error_retry_times):
			...
			async with session.get(locauri, headers=headers, allow_redirects=False) as res2:
				if res2.status in (302, 307):
					...
					await self.client.webhooks.send(
						content = self.client.data.discord.mention,
						title = "⚙️ submit HCAPTCHA OAUTH ⚙️",
	...
	async def detect_image_captcha(self, message):
		if not self.client.data.available.captcha and self.client.conditions.message(message, True, False, ['⚠️', 'letter word'], []) and (message.channel.id == self.client.data.owo.dm_channel.id or str(self.client.user.name) in message.content) and message.attachments:
	...
	async def detect_unknown_captcha(self, message):
		if not self.client.data.available.captcha and self.client.conditions.message(message, True, False, [f'<@{self.client.user.id}>', 'verify that you are human!'], []) and not message.attachments and not "https://owobot.com/captcha" in message.content:
			...
			self.client.data.available.captcha = True
			self.client.data.available.selfbot = False
			self.client.logger.warning(f"!!! Unknown Captcha appears !!!")
			await self.client.webhooks.send(
				content = self.client.data.discord.mention,
	...
	async def detect_problems(self, message):
		if self.client.conditions.message(message, True, False, ['You have been banned'], []) and (str(self.client.user.name) in message.content or message.channel.id == self.client.data.owo.dm_channel.id):
			await self.client.notification.notify()
